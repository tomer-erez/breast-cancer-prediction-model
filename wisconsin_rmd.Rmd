...
in our data, Features are computed from a digitized image of a fine needle aspirate (FNA) of a breast mass.
...
we studied those characteristics and provided our analysis.
...
![](C:\Users\97254\Documents\path for breast cancer data\log_eq.JPEG)
...
Y      <- c(pred_table[2], pred_table[1], pred_table[4], pred_table[3])
...
as we can see our predictions were mostly accurate both for TRUE NEGATIVE (malignancy) and TRUE POSITIVE (benign).
unfortunately, we do have some false positives(top right) and false negatives(bottom left).
...
This is an ideal situation. When two curves don’t overlap at all that means model has an ideal measure of separability. 
It is perfectly able to distinguish between positive class and negative class.
so our goal was to maximize the area under this ROC graph ,which worked out pretty well.
...
In this article we developed a linear regression model which appeared to be very effective in predicting malignancy of a tumor.
